...
The databricks CLI bundles top level is defined by file `databricks_mlops_stacks/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`databricks_mlops_stacks/assets/model-workflow-asset.yml`)
 - batch inference workflow (`databricks_mlops_stacks/assets/batch-inference-workflow-asset.yml`)
 - monitoring workflow (`databricks_mlops_stacks/assets/monitoring-workflow-asset.yml`)
 - feature engineering workflow (`databricks_mlops_stacks/assets/feature-engineering-workflow-asset.yml`)
 - model definition and experiment definition (`databricks_mlops_stacks/assets/ml-artifacts-asset.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `databricks_mlops_stacks/databricks.yml` and `databricks_mlops_stacks/assets/ml-artifacts-asset.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML assets to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-databricks-mlops-stacks-model     | /dev-databricks-mlops-stacks-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-databricks-mlops-stacks-model | /staging-databricks-mlops-stacks-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-databricks-mlops-stacks-model    | /prod-databricks-mlops-stacks-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML assets deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-databricks-mlops-stacks-model    | /test-databricks-mlops-stacks-experiment    |
...
2. Complete the `TODO` in `databricks_mlops_stacks/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `databricks_mlops_stacks/assets/batch-inference-workflow-asset.yml` to pass
...
To get started, open `databricks_mlops_stacks/assets/batch-inference-workflow-asset.yml`.  The file contains the ML asset definition of a batch inference job, like:
...
The example above defines a Databricks job with name `${bundle.target}-databricks-mlops-stacks-batch-inference-job`
that runs the notebook under `databricks_mlops_stacks/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `assets/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-databricks-mlops-stacks-batch-inference-job`.
...
`test-databricks-mlops-stacks-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-databricks-mlops-stacks-batch-inference-job`.
