...
* Feature computation modules under `databricks_mlops_stacks/feature_engineering` folder. 
...
* Python unit tests for feature computation modules in `databricks_mlops_stacks/tests/feature_engineering` folder.
* Feature engineering notebook, `databricks_mlops_stacks/feature_engineering/notebooks/GenerateAndWriteFeatures.py`, that reads input dataframes, dynamically loads feature computation modules, executes their `compute_features_fn` method and writes the outputs to a Feature Store table (creating it if missing).
...
* An automated integration test is provided (in `.github/workflows/databricks-mlops-stacks-run-tests-fs.yml`) that executes a multi task run on Databricks involving the feature engineering and model training notebooks.
...
1. Implement your feature module, address TODOs in `databricks_mlops_stacks/feature_engineering/features` and create unit test in `databricks_mlops_stacks/tests/feature_engineering`
2. Update `databricks_mlops_stacks/assets/feature-engineering-workflow-asset.yml`. Fill in notebook parameters for `write_feature_table_job`.
3. Update training data path in `databricks_mlops_stacks/assets/model-workflow-asset.yml`.

We expect most of the development to take place in the `databricks_mlops_stacks/feature_engineering` folder.
...
You can iterate on ML code by running the provided `databricks_mlops_stacks/feature_engineering/notebooks/GenerateAndWriteFeatures.py` notebook on Databricks using
...
#### Prerequisites
* Python 3.8+
* Install feature engineering code and test dependencies via `pip install -I -r databricks_mlops_stacks/requirements.txt -r test-requirements.txt` from project root directory.
